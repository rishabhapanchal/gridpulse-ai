...
async function startServer() {
  ...
  // Lazy initialize Gemini client on the server side to protect startup integrity
...
Provide a high-fidelity estimation of how much they could save by adapting rooftop solar panel arrays.`,
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              ...
              summaryOfFindings: {
                type: Type.STRING,
                description: 'A 2-sentence summary of the bill observation (e.g. "We noted heavy peak hour usage charges in tier 2 pricing. Your roof is ideally situated to mitigate this...").',
              },
            },
            required: [
              'extractedBillAmount',
  ...
  app.post('/api/chat', async (req, res) => {
    try {
      ...
      // Convert UI message history to @google/genai Chat/Contents standards
  ...
  // Handle Vite Asset pipeline based on environment
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`Server environment booted. Listening on port ${PORT}`);
  });
}
